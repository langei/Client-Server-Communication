...
def threadedClient(connection):                                #When a new thread is started, threadedClient code begins
    connection.sendall(str.encode('Welcome to the Server'))    #Sends an encoded message to the client
    while True:
        data = connection.recv(1024)                           #Waits to receive a message
        if data.decode('utf-8') == '0':                        #If the client is properly connected (i.e.sends '0'), the server sends them a command
...
def sendText(message):                          #Function to send a message to all clients
    encodedMessage = bytes(message, 'utf-8')    #Encodes the message given in the function parameter
